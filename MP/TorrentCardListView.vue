...
// 排序
const sortDataList = (list: SearchTorrent[], sortField: string): SearchTorrent[] => {
  return list.sort((a, b) => {
    switch (sortField) {
      case 'default':
        return (b.torrent_info.pri_order || 0) - (a.torrent_info.pri_order || 0);
      case 'site':
        return (a.torrent_info.site_name || '').localeCompare(b.torrent_info.site_name || '');
      case 'size':
        return (b.torrent_info.size || 0) - (a.torrent_info.size || 0);
      case 'seeder':
        return (b.torrent_info.seeders || 0) - (a.torrent_info.seeders || 0);
      default:
        return 0;
    }
  });
...
function filterData() {
  ...
  // 存储所有匹配的数据
  const filteredDataList: SearchTorrent[] = []
  ...
  groupedDataList.value?.forEach(value => {
    if (value.length > 0) {
      ...
      if (matchData.length > 0) {
        ...
        filteredDataList.push(firstData)
      }
    }
  })

  // 对过滤后的数据进行排序
  const sortedDataList = sortDataList(filteredDataList, filterForm.sortField)

  // 显示前20个，4行左右。
  sortedDataList.forEach((item, index) => {
    if (index < 20) {
      displayDataList.value.push(item)
    } else {
